...
class BNFeatureHook_new():
    ...
    def __init__(self, module, per_class_bn=False, ipc=None, start=None, end=None):
        self.hook = module.register_forward_hook(self.hook_fn)
        self.per_class_bn = per_class_bn
        self.ipc = ipc
        self.start = start
        self.end = end
        print('per-class-batchnorm: ', per_class_bn)
    ...
    def hook_fn(self, module, input, output):
        if self.per_class_bn:
            ...
            # print(f'Mean and var shape: {mean.shape}, {var.shape}')

            # r_feature = torch.norm(module.running_var_per_class.data[self.start: self.end] - var, 2) + torch.norm(
            # module.running_mean_per_class.data[self.start: self.end] - mean, 2)
    ...
    def hook_fn(self, module, input, output):
        if self.per_class_bn:
            if input[0].ndim == 4:
                ...
                mean = input[0].mean([1, 2]).unsqueeze(dim=-1)
                var = input[0].var([1, 2], unbiased=False).unsqueeze(dim=-1)
...
class BatchNorm1dWithPerClassStats(nn.BatchNorm1d):
    def __init__(self, num_features, num_classes, *args, **kwargs):
        super(BatchNorm1dWithPerClassStats, self).__init__(num_features, *args, **kwargs)
    ...
    def forward(self, input, labels=None):
        if labels is not None and not self.training:
            ...
            with torch.no_grad():
                ...
                class_mean = input.mean()
                class_var = input.var(unbiased=False)
        ...
        B, N, C = input.shape
        x = input.reshape(B * N, C)  # (B,N,C) -> (B*N,C)
        x = super(BatchNorm1dWithPerClassStats, self).forward(x)  # apply batch normalization
...
def replace_batchnorm(module, num_classes):
    ...
    for name, child in module.named_children():
        ...
        if isinstance(child, nn.BatchNorm1d):
            setattr(module, name, BatchNorm1dWithPerClassStats(child.num_features, num_classes))
        else:
